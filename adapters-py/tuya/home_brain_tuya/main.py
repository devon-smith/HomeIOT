"""Tuya adapter — bridges MQTT to a Backend (mock or real tinytuya).

Topics:
  - Subscribes to home/{room}/{device}/command for every (room, device) in
    house.yaml where adapter == tuya.
  - For climate devices (kind=climate): publishes the climate state payload
    on home/{room}/{device}/state.
  - For switch devices (kind=switch): publishes `{on: bool}` on
    home/{room}/{device}/state.
  - Heartbeats home/_meta/adapter/tuya/health every 15s with LWT.
"""
...
from .backend import Backend, ClimateState, DeviceConfig, SwitchState
from .mock_backend import MockBackend
from .tinytuya_backend import TinyTuyaBackend
...
def load_devices() -> list[DeviceConfig]:
    ...
    for room_slug, room in (data.get("rooms") or {}).items():
        for device_slug, dev in (room.get("devices") or {}).items():
            if dev.get("adapter") != "tuya":
                continue
            cfg = dev.get("config") or {}
            kind = cfg.get("kind") or _infer_kind(device_slug)
            out.append(
                DeviceConfig(
                    room=room_slug,
                    device=device_slug,
                    kind=kind,
                    tuya_id=cfg.get("device_id", ""),
...
def _infer_kind(device_slug: str) -> str:
    climate_slugs = {"sauna", "hot_tub", "pool", "hvac"}
    return "climate" if device_slug in climate_slugs else "switch"
...
class Adapter:
    def __init__(self, backend: Backend, devices: list[DeviceConfig], mqtt_url: str) -> None:
        ...
        self._device_kind: dict[tuple[str, str], str] = {
            (d.room, d.device): d.kind for d in devices
        }
    ...
    async def _dispatch(self, room: str, device: str, cmd: dict[str, Any]) -> None:
        ...
        kind = self._device_kind.get((room, device))
        log.info("command room=%s device=%s kind=%s op=%s id=%s", room, device, kind, op, cmd_id)
        try:
            if kind == "climate":
                if op == "set_target":
                    ...
                    raise ValueError(f"unsupported climate op: {op}")
            elif kind == "switch":
                if op == "set":
                    state = await self.backend.set_on(room, device, bool(args["on"]))
                else:
                    raise ValueError(f"unsupported switch op: {op}")
            else:
                raise ValueError(f"unknown device: {room}.{device}")
    ...
    def _publish_state(
        ...
        room: str,
        device: str,
        state: ClimateState | SwitchState,
        cmd_id: str | None,
    ) -> None:
    ...
    def _on_external_change(
        self, room: str, device: str, state: ClimateState | SwitchState
    ) -> None:
...
async def amain() -> int:
    mode = os.environ.get("TUYA_MODE", "mock").lower()
    mqtt_url = os.environ.get("MQTT_URL", "mqtt://localhost:1883")
    log.info("starting tuya adapter mode=%s mqtt=%s", mode, mqtt_url)

    devices = load_devices()
    if not devices:
        log.error("no devices in house.yaml use the tuya adapter")
        return 1
    log.info(
        "managing %d devices: %s",
        len(devices),
        [(d.room, d.device, d.kind) for d in devices],
    )

    backend: Backend = MockBackend() if mode == "mock" else TinyTuyaBackend()
